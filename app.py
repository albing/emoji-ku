...
def random_emoji(unicode_version = 6):
    ...
    #emoji_codepoint = "\\U000{}".format(hex(ord(emoji))[2:].upper())

    return emoji#_codepoint

print(random_emoji(UNICODE_VERSION))

app = Flask(__name__)

def get_rand(how_many):
	s=""
...
def haiku():
	...
	poem = get_rand(5) + "\n" + get_rand(7) + "\n" + get_rand(5)
	return Response('{"response_type": "in_channel", "text": "%s"}' % poem, mimetype='application/json')
